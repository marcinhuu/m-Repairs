...
CreateThread(function()
    for _, item in pairs(Config.Peds) do
        ...
        if item.ped == nil then
            local ped = CreatePed(item.type, item.hash, item.vector4.x, item.vector4.y, item.vector4.z-1, item.vector4.w, false, true)
...
CreateThread(function ()
    for k, v in pairs(Config["Repairs"]) do
        ...
        if Config.Function == 'colour' then
            label = Config.Language.paint
        elseif Config.Function == 'repair' then
            label = Config.Language.repair
...
function RepararCarro()
    QBCore.Functions.Progressbar("RepararCarro", Config.Language.progressRepair, 5000, false, true, {disableMovement = true,disableCarMovement = true,disableMouse = false,disableCombat = true}, {}, {}, {}, function() 
    end)
end

function PintarCarro()
    QBCore.Functions.Progressbar("PintarCarro", Config.Language.progressPaint, 5000, false, true, {disableMovement = true,disableCarMovement = true,disableMouse = false,disableCombat = true}, {}, {}, {}, function() 
    end)
end
...
AddEventHandler("m-Repairs:client:Reparar", function()
    ...
    if not vehicle then
        QBCore.Functions.Notify(Config.Language.insideVeh, "error")
        return
    end
